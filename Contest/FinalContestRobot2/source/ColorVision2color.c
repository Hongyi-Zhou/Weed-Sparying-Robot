...
/////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////
//start necessary variables for collecting blue and orange centroid data
...
volatile int noimagefound = 1;
volatile float orange_x = 0.0;
volatile float orange_y = 0.0;
volatile int orange_numpels = 0;
volatile float blue_x = 0.0;
volatile float blue_y = 0.0;
volatile int blue_numpels = 0;
volatile int new_coordata = 0;
volatile int new_num_found_objects = 0;

//end modified variables for storing blue and orange data
/////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////
...
void userProcessColorImageFunc_laser(bgr *ptrImage) {
	SETLED4;		//start of color thresholding
...
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
		//for loop begins here (for loop executes twice, once to look for blue, then again to look for orange)
		int colorCount;
		for(colorCount = 0; colorCount<2; colorCount++)
		{

////////////////////////////////////////////////////////////////////////////////////////////
//			//optitrack color threshold values
//			if (colorCount == 0)
//			{											//searching for blue
//				//Selected Threshold: H [153:176], S [79:255], V [126:255]
//				specs_h = 165;
//				specs_hrad = 11;
//				if((specs_h-specs_hrad)<0) { // wrap 0->360
//					specs_h2=specs_h+256;
...
///////////////////////////////////////////////////////////////////////////////////////////////
			...
			//orange no optitrack threshold: Selected Threshold: H [0:71,244:255], S [239:255], V [187:255]
			//blue no optitrack threshold: Selected Threshold: H [94:156], S [57:255], V [51:255]
			//orange: specs_h = 30, specs_hrad = 41;
					//specs_s = 247, specs_srad = 8
					//specs_v = 221, specs_vrad = 34;
			//blue: specs_h = 125, specs_hrad = 31;
					//specs_s = 156, specs_srad = 99
					//specs_v = 153, specs_vrad = 102;

			//non-optitrack color thresholds
			//depending on which loop in the for loop, the thresholds must change to look for different colors
			if (colorCount == 0)				//in the first time through the loop, set the thresholds for blue
			{											//searching for blue
				//Selected Threshold: H [153:176], S [79:255], V [126:255]
				specs_h = 125;
				specs_hrad = 31;
			...
			else			//in the second time through the loop, change the thresholds for orange
			{										//searching for orange
				specs_h = 30;
				specs_hrad = 41;
				if((specs_h-specs_hrad)<0) { // wrap 0->360
...
///////////////////////////////////////////////////////////////////////////////////////////
			...
			//need to clear max_num_equivalencies between looking at blue and orange
			//code for looking thresholding the image and determining object stats the same between blue and orange
			...
			for (r=0;r<IMAGE_ROWS-(IMAGE_ROWS/3);r++) {			//don't go all the way to the top row, so don't check lights
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////
//		//old passing of new coordinate data from when only one color was being searched for
...
//					blue_x = cbar - IMAGE_COLUMNS/2;
//					blue_y = rbar - IMAGE_ROWS/2;
//					blue_numpels = final_object_stats[largest_object].num_pixels_in_object;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//modified to support doing sending back of two colors instead of just one
			// pass data to RobotControl()
			if (new_coordata == 0)
			{
				if (final_object_stats[largest_object].num_pixels_in_object > 1)	//if an object is found, blue or orange
				{
					noimagefound = 0;
					new_num_found_objects = num_unique_objects;
					if (colorCount == 1)		//found an orange object, so update orange object data to be sent back to RobotControl()
					{
						orange_x = cbar - IMAGE_COLUMNS/2;
						orange_y = rbar - IMAGE_ROWS/2;
						orange_numpels = final_object_stats[largest_object].num_pixels_in_object;//blue object detected
	//					blue_x = cbar - IMAGE_COLUMNS/2;
	//					blue_y = rbar - IMAGE_ROWS/2;
	//					blue_numpels = final_object_stats[largest_object].num_pixels_in_object;
						//new_object_theta 	= final_object_stats[largest_object].theta;
						//new_C20				= final_object_stats[largest_object].C20_sum;
						//new_C02				= final_object_stats[largest_object].C02_sum;
						new_coordata = 1;	//put this here since this is in a loop, don't want to raise the new_coordata flag until after both orange and blue are processed, so when colorCount == 1
					}
					else			//colorCount ==0, found a blue object so update blue object data to be sent back to RobotControl(),
					{					//don't raise the new_coordata flag yet because need to process image again to look for orange
						blue_x = cbar - IMAGE_COLUMNS/2;
						blue_y = rbar - IMAGE_ROWS/2;
						blue_numpels = final_object_stats[largest_object].num_pixels_in_object;
	...
	//					orange_x = cbar - IMAGE_COLUMNS/2;
	//					orange_y = rbar - IMAGE_ROWS/2;
	//					orange_numpels = final_object_stats[largest_object].num_pixels_in_object;
						//new_object_theta 	= final_object_stats[largest_object].theta;
						//new_C20				= final_object_stats[largest_object].C20_sum;
						//new_C02				= final_object_stats[largest_object].C02_sum;
					}
				...
				{
					noimagefound = 1;
					new_num_found_objects = num_unique_objects;		//no object found, either blue or orange
					if (colorCount == 0)				//searched for blue but didn't find object, so set blue object data to 0 to be sent back to RobotControl(),
					{									//don't raise the new_coordata flag yet because still need to process orange
						blue_x = 0.0;
						blue_y = 0.0;
						blue_numpels = 0;
						//new_object_theta = 0.0;
					...
					{									//colorCount == 1, searched for orange but didn't find an object, so set orange object data to 0 to be sent back to RobotControl()
						orange_x = 0.0;
						orange_y = 0.0;
						orange_numpels = 0;
						//new_object_theta = 0.0;
						//new_C20 = 0.0;
						//new_C02 = 0.0;
						new_coordata = 1;	//put this here since this is in a loop, don't want to raise the new_coordata flag until after both orange and blue are processed, so colorCount == 1
					}
				}
			}
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//this will still execute twice once each time through the loop, so a green x should be marked for both the largest blue (first pass) and for the largest orange (second pass)
        ...
        //for loop ends here because rest of code updates the image on the LCD screen, and that process only needs to happen once after both orange and blue are thresholded
///////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	CLRLED4;				//end of color thresholding
